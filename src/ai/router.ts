...
import { createRouter, type LlmRouter, type RouterConfig } from '@doozhub/ai-router';
...
export function getRouter(): LlmRouter | null {
    ...
    try {
        ...
        console.log('[ai] Router initialized with providers:', config.providers.map((p: { type: string }) => p.type).join(', '));
        return router;
    } catch (error) {
